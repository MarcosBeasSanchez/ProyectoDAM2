...
import { listarRequest } from "../api/Request.js";
...
function ListPage() {
    ...
    useEffect(() => {
        const listarUsuarios = async () => {
            try {
                const response = await listarRequest(); // Llamada a la función listarRequest
                setListaUsuarios(response.data.usuarios); // Guardamos los usuarios en el estado
            } catch (error) {
                console.error("Error al obtener los usuarios:", error);
            }
        };

        listarUsuarios(); // Ejecuta la función cuando el componente se monte
    }, []);
    ...
    return (
        <div>
                        ...
                        <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start ">
                            ...
                            <div className="hidden sm:ml-6 sm:block">
                                <div className="flex space-x-4">
                                    <a href="http://localhost:5173/register" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white" aria-current="page">Register</a>
                                    <a href="http://localhost:5173/login" className="rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-gray-700 hover:text-white">Login</a>
                                    ...
                                    <a href="http://localhost:5173/list" className="rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white" aria-current="page">List</a>
                                </div>
                            </div>
                        </div>
            ...
            <h1 className=' text-blue-950 text-4xl text-center font-bold mt-10'>USUARIOS</h1>

            <div className='flex flex-col gap-5 flex-wrap mt-10  justify-items-center p-5'>
                {listaUsuarios.map((usuario) => (
                    <div className='p-4 border rounded-lg shadow-md bg-indigo-200'>
                        <h2 className='text-lg font-bold'> 
                            Username: {usuario.username}</h2>
